...
// Added updatePlaylist helper to update playlist metadata or tracks
export const updatePlaylist = (updatedPlaylist) => {
  const playlists = getPlaylists();
  const newPlaylists = playlists.map(p =>
    p.id === updatedPlaylist.id
      ? { ...p, ...updatedPlaylist, updatedAt: new Date().toISOString() }
      : p
  );
...
export const importData = (data) => {
  ...
  if (data.recentlyPlayed) {
    try {
      ...
      console.error('Error importing recently played:', error);
    }
  }
};
